...
Partial Class Customer_Customer_CreditRec
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        Try
            If Val(txtfrom.Text) = 0 Then
                ...
                btnrecre.Visible = False
                btnrecview.Visible = False
    ...
    Sub Credit_Due_Rec()
        ...
        Dim rec_from_Credit As String
        Dim rec_to_Credit As String
        rec_from_Credit = Date.Parse(txtfrom.Text).ToString("yyyy-MM-dd")
        Session("rec_from_Credit") = rec_from_Credit
        rec_to_Credit = Date.Parse(txtto.Text).ToString("yyyy-MM-dd")
        Session("rec_to_Credit") = rec_to_Credit
        ...
        If ds_rec.Tables(0).Rows.Count = 0 Then
        ...
        Else
            trm.Cells.Add(tcm)
            tbl.Rows.Add(trm)
            lbl_head.Text = "View Credit Received Report from " & txtfrom.Text.Replace("-", " ") & " to " & txtto.Text.Replace("-", " ")
            lbl_head.Visible = True
            lbl_head.Style.Add(" font-weight", "bold")
            btnrecre.Visible = True
            btnrecview.Visible = True
            Session("cre_rec_count") = ds_rec.Tables(0).Rows.Count
        End If
        ...
        For i As Integer = 0 To ds_rec.Tables(0).Rows.Count - 1
            ...
            lbl_cust_no.Text = Session("Branch_Prefix") & " " & ds_rec.Tables(0).Rows(i).Item("Customer_ID")
            ...
            lbl_cust_name.Text = ds_rec.Tables(0).Rows(i).Item("Given_Name") & "  " & ds_rec.Tables(0).Rows(i).Item("Last_Name")
            ...
            lbl_loan_id.Text = ds_rec.Tables(0).Rows(i).Item("Loan_ID").ToString
            ...
            due_dt_new = ds_rec.Tables(0).Rows(i).Item("Due_Date").ToString()
            ...
            pay_dt_new = ds_rec.Tables(0).Rows(i).Item("Payment_Date").ToString()
            ...
            lbl_amt.Text = open_con.new_amount(amt_creditrec)
        ...
        lbl_totalamt.ID = "tot"
        tot_creditrec = open_con.check_amount_format(Math.Round(tot_creditrec, 2))
        lbl_totalamt.Text = open_con.new_amount(tot_creditrec)
        ...
        ds_rec.Dispose()
    End Sub
    Protected Sub btnrecre_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnrecre.Click
        txtfrom.Text = ""
        txtto.Text = ""
        lbl_head.Visible = False
        btnrecre.Visible = False
        btnrecview.Visible = False
    End Sub
    Protected Sub btnrecview_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnrecview.Click
        Response.Redirect("../Reports/Print_CreRec.aspx", False)
    End Sub
